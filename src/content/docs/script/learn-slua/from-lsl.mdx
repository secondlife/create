...
LLEvents:on("touch_start", function(events: {LLDetectedEvent})
    clickCount += 1
...
LLEvents:on("collision_start", function(events: {LLDetectedEvent})
    ll.Say(0, "Collision!")
end)
```
...
```luau
LLEvents:on("touch_start", function(events: {LLDetectedEvent})
    for i: number, event: LLDetectedEvent in events do
