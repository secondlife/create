...
local rot: quaternion = quaternion(0, 0, 0, 1) -- also local rot: rotation = rotation(0, 0, 0, 1)
...
#### Casting
In many cases, luau does casting automatically. Here's how to do it manually:
<CodeComparison>
<Fragment slot="lsl">
```lsl
(integer)"-4.4";
...
<Fragment slot="slua">
```luau
(math.modf("-4.4")) -- extra parentheses are necessary to discard the second return value
tonumber("25.5")
tostring(99.9)
tovector("<10, 20, 30>")
torotation("<0, 0, 0, 1>") -- also toquaternion("<0, 0, 0, 1>")
"700843d3-0d15-c427-81ab-e9c8a0dcdb1e"  -- keys are strings
