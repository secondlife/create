...
Notes:
1. <a name="band"/>`bit32.btest` returns a boolean; `bit32.band` returns an integer. Both match the behavior of LSL `&`.
    Integers (`bit32.band`) don't work inside `if` statements, unlike LSL. Use whichever is most appropriate.
2. <a name="s32"/>Hexadecimal numbers `0x8000000` and above are positive in Lua, but negative in LSL,
