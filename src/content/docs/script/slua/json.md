...
For serialising types like `vector`, `quaternion`, `uuid` you can use `lljson.slencode()`:

```luau
local data = {
    position = vector(1,2,3),
    rotation = quaternion(0,0,0,1),
    uuid = uuid("c8c30f77-626b-4f63-b5ca-c80c3b3cc6f0")
}

local json = lljson.slencode(data)
...
<!-- For JSON encoded with serialised types like `vector`, `quaternion`, `uuid` you can use `lljson.sldecode()`:

` ``luau
local json = '{}'
local data = lljson.sldecode(json)

ll.OwnerSay(`color: {data.color}`)
ll.Ownersay(`texture: {data.texture}`)
``` -->
